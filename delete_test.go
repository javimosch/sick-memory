...
func TestHandleDeleteByFullMemoryID(t *testing.T) {
	oldJSON := jsonOutput
	jsonOutput = true
	t.Cleanup(func() { jsonOutput = oldJSON })
	...
	os.Args = []string{"cmd", "delete", "memory_1"}
...
func TestHandleDeleteBySuffix(t *testing.T) {
	oldJSON := jsonOutput
	jsonOutput = true
	t.Cleanup(func() { jsonOutput = oldJSON })
	...
	if err := os.WriteFile(filepath.Join(dir, "project_123.md"), []byte("content"), 0644); err != nil {
		t.Fatalf("failed to write memory file: %v", err)
	}
	...
	if _, err := os.Stat(filepath.Join(dir, "project_123.md")); !os.IsNotExist(err) {
		t.Errorf("expected memory file to be deleted")
	}
}
