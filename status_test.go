...
func TestHandleStatusActiveWithMemoriesTextOutput(t *testing.T) {
	oldJSON := jsonOutput
	jsonOutput = false
	t.Cleanup(func() { jsonOutput = oldJSON })
	...
	if strings.Contains(got, "uninitialized") {
		t.Errorf("expected active status, got uninitialized output: %q", got)
	}
}
...
func TestHandleStatusActiveIgnoresSubdirectories(t *testing.T) {
	oldJSON := jsonOutput
	jsonOutput = true
	t.Cleanup(func() { jsonOutput = oldJSON })
...
func TestStatusMainShortJSON(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	...
	os.Args = []string{"sick-memory", "status", "-j"}
	jsonOutput = true
	memoryDir = dir
	noInteractive = false
...
func TestHandleStatusActiveWithHiddenFilesTextOutput(t *testing.T) {
	oldJSON := jsonOutput
	jsonOutput = false
	t.Cleanup(func() { jsonOutput = oldJSON })
	...
	if strings.Contains(got, "hidden") {
		t.Errorf("expected hidden memory to be ignored, got %q", got)
	}
}
