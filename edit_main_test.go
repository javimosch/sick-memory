...
func TestEditMainTextOutputTruncatesAtNewline(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
...
`)
	...
	os.Args = []string{"sick-memory", "edit", "1", "First line\nsecond line"}
	jsonOutput = false
	memoryDir = dir
	noInteractive = false
	...
	if !strings.Contains(string(updated), "description: First line") {
		t.Errorf("expected truncated description at newline, got:\n%s", string(updated))
	}
...
func TestEditMainByFullMemoryID(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
...
`)
	...
	os.Args = []string{"sick-memory", "edit", "memory_1", "full id update"}
	jsonOutput = true
	memoryDir = dir
	noInteractive = false
	...
	if data["description"] != "full id update" {
		t.Errorf("description = %v, want %q", data["description"], "full id update")
	}
	...
	if !strings.Contains(string(updated), "full id update") {
		t.Errorf("expected updated content in file, got:\n%s", string(updated))
	}
}

func TestEditMainSkipsGlobalFlags(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
...
`)
	...
	os.Args = []string{"sick-memory", "edit", "1", "--memory-dir", dir, "--json", "global flag content"}
	jsonOutput = false
	memoryDir = ""
	noInteractive = false
	...
	if strings.Contains(string(updated), "--memory-dir") || strings.Contains(string(updated), "--json") {
		t.Errorf("did not expect global flags in memory content, got:\n%s", string(updated))
	}
	if !strings.Contains(string(updated), "global flag content") {
		t.Errorf("expected updated content in file, got:\n%s", string(updated))
	}
}
