...
func TestHandleListWithMemoriesTextOutput(t *testing.T) {
	oldJSON := jsonOutput
	jsonOutput = false
	t.Cleanup(func() { jsonOutput = oldJSON })
	...
	if !strings.Contains(got, "memory_1.md") {
		t.Errorf("expected output to contain memory_1.md, got %q", got)
	}
	if !strings.Contains(got, "memory_2.md") {
		t.Errorf("expected output to contain memory_2.md, got %q", got)
	}
	if !strings.Contains(got, "Total memories: 2") {
		t.Errorf("expected output to contain 'Total memories: 2', got %q", got)
	}
}
...
func TestListMainMemoryDir(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
...
func TestHandleListIgnoresInvalidMemoryFilenames(t *testing.T) {
	oldJSON := jsonOutput
	jsonOutput = true
	t.Cleanup(func() { jsonOutput = oldJSON })
	...
	for _, name := range []string{"memory_1.md", "memory_2", "memory_3.txt", "README.md"} {
...
func TestMainListMemoryDirTextOutput(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	...
	os.Args = []string{"sick-memory", "list", "--memory-dir", dir}
	jsonOutput = false
	memoryDir = ""
	noInteractive = false
...
func TestListMainMemoryDirTextOutput(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
...
func TestLsMainMemoryDirTextOutput(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	...
	os.Args = []string{"sick-memory", "ls", "--memory-dir", dir}
	jsonOutput = false
	memoryDir = ""
	noInteractive = false
	...
	got := string(out)
	if !strings.Contains(got, "Memories in") {
		t.Errorf("expected 'Memories in' in output, got %q", got)
	}
	if !strings.Contains(got, "memory_1.md") {
		t.Errorf("expected memory_1.md in output, got %q", got)
	}
	if !strings.Contains(got, "Total memories: 1") {
...
func TestHandleListIgnoresSubdirectories(t *testing.T) {
	oldJSON := jsonOutput
	jsonOutput = true
	t.Cleanup(func() { jsonOutput = oldJSON })
