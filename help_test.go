...
func TestPrintHelpContainsAllCommandsAndOptions(t *testing.T) {
	r, w, err := os.Pipe()
	if err != nil {
		t.Fatalf("failed to create pipe: %v", err)
	}
...
func TestPrintHelpContainsShortFlags(t *testing.T) {
	r, w, err := os.Pipe()
	if err != nil {
		t.Fatalf("failed to create pipe: %v", err)
	}
...
func TestMainShortHelp(t *testing.T) {
	if os.Getenv("MAIN_SHORT_HELP") == "1" {
	...
	home := t.TempDir()
	cmd := exec.Command(os.Args[0], "-test.run=^TestMainShortHelp$", "-test.v")
	cmd.Env = append(os.Environ(), "MAIN_SHORT_HELP=1", "HOME="+home)
	out, err := cmd.CombinedOutput()
	if err != nil {
